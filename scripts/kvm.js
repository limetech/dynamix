...
$(function(){
   $('#cdrom_tree').fileTree(
	...
	$('#media_tree').fileTree({root:'/mnt/',filter:['iso','ISO'],script:'/plugins/dynamix.kvm.manager/classes/FileTree.php',multiFolder:false}, function(file) {$('#media_file').val(file);$('#media_tree').hide();}, function(directory) {$('#media_dir').val(directory);});
	$('#disk_tree').fileTree({root:'/mnt/',filter:['qcow2','qcow','img'],script:'/plugins/dynamix.kvm.manager/classes/FileTree.php',multiFolder:false}, function(file) {$('#disk_file').val(file);$('#disk_tree').hide();}, function(directory) {$('#disk_file').val(directory);});
	$('#new_disk_tree').fileTree({root:'/mnt/',filter:['.'],script:'/plugins/dynamix.kvm.manager/classes/FileTree.php',multiFolder:false}, function(file) {$('#new_disk_file').val(file);$('#new_disk_tree').hide();}, function(directory) {$('#new_disk_file').val(directory);});
